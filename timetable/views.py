...
from .views_extras import (
    ...
    notifications_mark_read, notifications_delete, api_unread_count, api_check_conflicts_advanced,
    api_check_lab_conflicts_advanced, public_schedule_export,
    student_schedule_pdf,
)
...
def _notify_students_schedule_change(schedule, action):
    """Send notifications to students and professors affected by a schedule change."""
    try:
        from .models import LabSchedule as _LabSchedule
        is_lab = isinstance(schedule, _LabSchedule)
        kind = 'معمل' if is_lab else 'محاضرة'
        day_label = DAY_NAMES.get(schedule.day_of_week, schedule.day_of_week)
        time_label = str(schedule.start_time)[:5]
        course_name = schedule.course.course_name

        action_msgs = {
            'add':    f'تمت إضافة {kind} جديد{"ة" if not is_lab else ""}: {course_name} - {day_label} {time_label}',
            'edit':   f'تم تعديل موعد {kind}: {course_name} - {day_label} {time_label}',
            'delete': f'تم حذف {kind}: {course_name}',
        }
        subject_msgs = {
            'add':    f'{kind} جديد{"ة" if not is_lab else ""} في جدولك',
            'edit':   f'تحديث في جدول ال{kind}ات',
            'delete': f'حذف {kind} من الجدول',
        }
        msg = action_msgs.get(action, f'تغيير في الجدول: {course_name}')
        ...
        prof_ids_to_notify = [schedule.professor_id]
        if is_lab and getattr(schedule, 'assistant_id', None):
            prof_ids_to_notify.append(schedule.assistant_id)

        for prof_id in prof_ids_to_notify:
            if not prof_id:
                continue
            try:
                prof_user = UnifiedUser.objects.get(user_type='professor', user_id=prof_id)
...
def require_system_manager(view_func):
    def wrapper(request, *args, **kwargs):
        if not request.user.is_authenticated:
            return redirect('login')
        if request.user.user_type != 'system_manager':
...
def require_college_manager(view_func):
    ...
    def wrapper(request, *args, **kwargs):
        if not request.user.is_authenticated:
            return redirect('login')
        if request.user.user_type not in ('college_manager', 'department_head'):
            messages.error(request, 'ليس لديك صلاحية للوصول لهذه الصفحة')
            return redirect('home')
        return view_func(request, *args, **kwargs)
    wrapper.__name__ = view_func.__name__
...
def require_college_manager_only(view_func):
    ...
    def wrapper(request, *args, **kwargs):
        if not request.user.is_authenticated:
            return redirect('login')
        if request.user.user_type != 'college_manager':
            messages.error(request, 'ليس لديك صلاحية للوصول لهذه الصفحة')
            return redirect('home')
        return view_func(request, *args, **kwargs)
    wrapper.__name__ = view_func.__name__
...
def check_lecture_conflicts(department_id, professor_id, room_id, day, start_time, end_time, exclude_id=None):
    ...
    # Professor conflict — check both lecture and lab schedules
    prof_conflict = qs.filter(professor_id=professor_id).select_related('course', 'department').first()
    if not prof_conflict:
        # Cross-check: professor might be teaching a lab at the same time
        prof_conflict = LabSchedule.objects.filter(
            day_of_week=day,
            start_time__lt=end_time,
            end_time__gt=start_time,
        ).filter(
        ...
        if prof_conflict:
            conflicts.append({
                'type': 'professor',
                'message': f'تعارض للأستاذ مع معمل: {prof_conflict.course.course_name} - قسم {prof_conflict.department.name}'
            })
    else:
        conflicts.append({
            'type': 'professor',
...
def system_manager_dashboard(request):
    ...
    recent_changelog = ScheduleChangeLog.objects.select_related('changed_by').order_by('-changed_at')[:5]
...
def university_add(request):
    if request.method == 'POST':
        ...
        form = UniversityForm()
    return render(request, 'timetable/university_form.html', {'form': form, 'title': 'إضافة جامعة'})
...
def university_edit(request, pk):
    ...
    return render(request, 'timetable/university_form.html', {'form': form, 'title': 'تعديل الجامعة', 'university': university})


@require_system_manager
def university_profile(request, pk):
    university = get_object_or_404(University, pk=pk)
    branches = university.branches.prefetch_related('colleges').order_by('-is_main', 'name')

    college_qs = College.objects.filter(branch__university=university)
    college_ids = list(college_qs.values_list('id', flat=True))
    college_count = len(college_ids)
    dept_count = CollegeDepartment.objects.filter(
        college_id__in=college_ids
    ).values('department').distinct().count()

    prof_count    = UnifiedUser.objects.filter(college_id__in=college_ids, user_type='professor').count()
    student_count = UnifiedUser.objects.filter(college_id__in=college_ids, user_type='student').count()
    cm_count      = UnifiedUser.objects.filter(college_id__in=college_ids, user_type='college_manager').count()
    dh_count      = UnifiedUser.objects.filter(college_id__in=college_ids, user_type='department_head').count()

    return render(request, 'timetable/university_profile.html', {
...
def user_list_admin(request):
    ...
    return render(request, 'timetable/users_admin.html', {
        'users': users,
...
@require_system_manager
def user_profile_admin(request, pk):
    profile_user = get_object_or_404(UnifiedUser, pk=pk)
    u_type = profile_user.user_type
    ...
    student_lectures = None

    if u_type == 'professor':
        try:
            professor = Professor.objects.select_related('college').get(username=profile_user.username)
            lectures  = LectureSchedule.objects.select_related('course', 'department', 'room').filter(professor=professor).order_by('day_of_week', 'start_time')
            labs      = LabSchedule.objects.select_related('course', 'hall').filter(professor=professor).order_by('day_of_week', 'start_time')
            change_requests = AlternativeTime.objects.filter(professor=professor).order_by('-created_at')[:15]
            lectures_count  = lectures.count()
            labs_count      = labs.count()
            requests_count  = AlternativeTime.objects.filter(professor=professor).count()
        except Professor.DoesNotExist:
            pass

    elif u_type == 'student':
        try:
            student = Student.objects.select_related('department', 'period').get(username=profile_user.username)
            if student.period:
                student_lectures = LectureSchedule.objects.select_related(
                    'course', 'professor', 'room'
                ).filter(period=student.period).order_by('day_of_week', 'start_time')
        except Student.DoesNotExist:
            pass

    notifications = Notification.objects.filter(recipient=profile_user).order_by('-created_at')[:20]
    notifs_count  = Notification.objects.filter(recipient=profile_user).count()

    return render(request, 'timetable/user_profile_admin.html', {
    ...
    if is_dept_head:
        stats = {
            ...
            'pending_requests': AlternativeTime.objects.filter(
                Q(schedule__department_id__in=dept_ids) |
                Q(schedule__isnull=True, professor__college_id=college.id),
                status='pending'
            ).count(),
        }
...
def cm_academic_periods(request):
    ...
    return render(request, 'timetable/cm_academic_periods.html', {
        'college': college, 'periods': periods, 'departments': departments,
    })
...
def cm_instructor_add(request):
    ...
    if request.method == 'POST':
        ...
        form.fields['college'].queryset = College.objects.filter(id=college.id)
        if form.is_valid():
            try:
                with transaction.atomic():
        ...
        form = ProfessorForm(initial={'college': college.id})
        form.fields['college'].queryset = College.objects.filter(id=college.id)
...
def cm_room_add(request):
    ...
    if request.method == 'POST':
        ...
        form = RoomForm(initial={'college': college.id})
        form.fields['college'].queryset = College.objects.filter(id=college.id)
...
def cm_hall_add(request):
    ...
    if request.method == 'POST':
        ...
        form = HallForm(initial={'college': college.id})
        form.fields['college'].queryset = College.objects.filter(id=college.id)
...
@require_college_manager_only
def cm_student_edit(request, pk):
    college = get_college_for_manager(request.user)
    college_depts = get_accessible_dept_ids(request.user)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            updated = form.save()
            unified = UnifiedUser.objects.filter(user_type='student', user_id=pk).first()
            if unified:
                unified.full_name = updated.name or ''
                unified.email = updated.email or ''
                unified.department = updated.department
                unified.save()
        ...
        form.fields['period'].queryset = DepartmentAcademicPeriod.objects.filter(department_id__in=college_depts)
    return render(request, 'timetable/form.html', {
        'form': form, 'title': 'تعديل بيانات الطالب', 'back_url': 'cm_student_list'
    })
...
def cm_request_approve(request, pk):
    college = get_college_for_manager(request.user)
    dept_ids = get_accessible_dept_ids(request.user)
    alt_time = get_object_or_404(
        AlternativeTime, pk=pk
    )
    if not (
        (alt_time.schedule_id and alt_time.schedule.department_id in dept_ids) or
        (alt_time.schedule_id is None and alt_time.professor.college_id == college.id)
    ):
...
def cm_report_professor_schedule(request):
    ...
    if prof_id:
        ...
        for lec in lectures:
            day_key = lec.day_of_week.title()
            if day_key in schedule_grid:
                schedule_grid[day_key].append({'type': 'lecture', 'obj': lec})
        for lab in labs:
            day_key = lab.day_of_week.title()
            if day_key in schedule_grid:
                schedule_grid[day_key].append({'type': 'lab', 'obj': lab})
...
def cm_notifications(request):
    ...
    return render(request, 'timetable/cm_notifications.html', {
        'college': college, 'notifications': notifications, 'unread_count': unread_count,
    })
...
def account_settings_view(request):
    ...
    if request.method == 'POST':
        ...
        if action == 'profile':
            ...
            if not new_username:
                profile_errors['username'] = 'اسم المستخدم مطلوب'
            elif ' ' in new_username:
                profile_errors['username'] = 'اسم المستخدم لا يجب أن يحتوي على مسافات'
            elif not re.match(r'^[\w.@+-]+$', new_username):
                profile_errors['username'] = 'اسم المستخدم يجب أن يحتوي على أحرف وأرقام وشرطة سفلية فقط'
            elif new_username != user.username and UnifiedUser.objects.filter(username=new_username).exists():
                profile_errors['username'] = 'اسم المستخدم هذا مستخدم بالفعل، الرجاء اختيار اسم آخر'
            ...
            if not profile_errors:
                ...
                user.email     = email
                user.save(update_fields=['username', 'full_name', 'email'])
...
@require_college_manager_only
def cm_dept_head_edit(request, pk):
    college = get_college_for_manager(request.user)
    dept_head = get_object_or_404(UnifiedUser, pk=pk, user_type='department_head', college=college)
    dept_ids = get_accessible_dept_ids(request.user)
    departments = Department.objects.filter(id__in=dept_ids)
    if request.method == 'POST':
        full_name = request.POST.get('full_name', '').strip()
        ...
        if errors:
            ...
            dept = get_object_or_404(Department, pk=dept_id)
            dept_head.full_name = full_name
            dept_head.email = email or dept_head.email
            dept_head.department = dept
            if new_password:
                dept_head.password = bcrypt.hashpw(new_password.encode(), bcrypt.gensalt()).decode()
            dept_head.save()
            messages.success(request, f'تم تحديث بيانات رئيس القسم {full_name} بنجاح')
            return redirect('cm_dept_head_list')
    return render(request, 'timetable/cm_dept_head_edit.html', {
        'college': college,
        'dept_head': dept_head,
        'departments': departments,
    })
...
def api_public_colleges(request):
    """Return colleges for a university — public, no login needed."""
    university_id = request.GET.get('university_id')
    if not university_id:
        return JsonResponse({'colleges': []})
    colleges = College.objects.filter(
        branch__university_id=university_id
    ).select_related('branch').distinct().order_by('name')
    return JsonResponse({'colleges': [{'id': c.id, 'name': c.name, 'code': c.code} for c in colleges]})
...
def cm_lecture_inline_edit(request, pk):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            _notify_students_schedule_change(schedule, 'edit')
